...
const { brand, copy, sponsors } = siteConfig
    ...
    <main>
      <section className="mx-auto max-w-content px-4 py-20 text-center sm:px-6 sm:py-28 lg:px-8 lg:py-36">
        <h1 className="text-3xl font-bold text-text-primary sm:text-4xl lg:text-5xl">
          {copy.openSourcePageHero}
        </h1>
        <p className="mx-auto mt-6 max-w-narrow text-base leading-relaxed text-text-secondary">
          {brand.name} is free, open source, and MIT licensed. The code is on GitHub. Read it, fork it,
          or build on top of it. Open source tools survive because people invest in them. If{' '}
          {brand.name} saves you time, consider supporting its development.
        </p>
      ...
      <section className="bg-surface px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto max-w-content">
          <h2 className="text-center text-2xl font-bold text-text-primary">Support this project</h2>
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {sponsors.map((sponsor) => (
              ...
              >
                ...
                <p className="mt-2 flex-1 text-sm text-text-secondary">{sponsor.description}</p>
      ...
      <section className="mx-auto max-w-content px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto max-w-narrow text-center">
          <h2 className="text-2xl font-bold text-text-primary">Contributors</h2>
          <p className="mt-4 text-base leading-relaxed text-text-secondary">
            {brand.name} exists because of open source contributors. Every bug report, pull request, and
            suggestion matters.
          </p>
          <Button asChild variant="secondary" className="mt-6">
            <a href={`${brand.githubUrl}/graphs/contributors`} target="_blank" rel="noreferrer">
              View Contributors on GitHub <ExternalLink size={16} aria-hidden="true" />
            </a>
          </Button>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-content px-4 pb-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-narrow">
          ...
          <p className="mt-4 text-base leading-relaxed text-text-secondary">
            Read CONTRIBUTING.md in the repository for guidelines on reporting bugs, suggesting features,
            and submitting pull requests.
          </p>
          <div className="mt-8 space-y-6 text-base text-text-secondary">
            <p>{copy.spreadTheWord}</p>
          </div>
          ...
          <p className="mt-10 text-center text-xs text-text-muted">MIT licensed. No warranties. Built with care.</p>
        </div>
      </section>
    </main>
  )
